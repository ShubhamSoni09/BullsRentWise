  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && usedStreetViewFallback && photos.length > 0 && (
        <div className="flex items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50 p-3 text-xs text-blue-700">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="empty-state py-12">
          ...
          {missingApiKey ? (
            <p className="text-amber-600 text-xs mt-2 font-medium max-w-xs mx-auto">
              Add <code>GOOGLE_API_KEY</code> to your <code>.env.local</code> file to enable Google Places and Street View photos.
            </p>
          ) : (
            <>
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {photos.map((photo) => (
              <div
                key={photo.id}
                className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:border-blue-200 hover:shadow-md"
              >
          ...
          {/* Photo count */}
          <div className="flex flex-col gap-1 rounded-lg bg-gray-50 p-3 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
